...
function delete_service(service_primary_key) {
	...
	if (confirm('are you sure you want to remove this service?') == true) {
		$.ajax({
			...
			data : {
				service_pk : service_primary_key
					...
					if (json.result == 'true') {
						...
						$('#list_service')
								.append(
										'<li id="service-'
												+ json.id
												+ '"><a class="edit_service">'
												+ json.name
												+ '</a><input class="delete_service" type="button" name="submit" id="delete-service-'
												+ json.id
												+ '" value="Delete"></li>'); // hide the post on success
...
function get_list_service(filter_service) {
	...
	$.ajax({
		url : "get_list_service/" + filter_service + "/", // the endpoint
		type : "POST", // http method
		data : {
			service_pk : service_primary_key
		}, // data sent with the delete request
